...
class EnvVarReplacerTest {
  ...
  fun replaceEnvVarsShouldReplaceEnvVarInName() {
    val config = Config(listOf(Profile($$"${name1}s", "root")))
  ...
  fun replaceEnvVarsShouldReturnErrorIfInvalidName() {
    val config = Config(listOf(Profile($$"${exception}s", "root")))
    val result = EnvVarReplacer.replaceEnvVars(config)
  ...
  fun replaceEnvVarsShouldReplaceEnvVarInRoot() {
    val config = Config(listOf(Profile("name", $$"${root1}s")))
    val result = EnvVarReplacer.replaceEnvVars(config)
    assertThat(result.exceptionOrNull()).isNull()
  ...
  fun replaceEnvVarsShouldReturnErrorIfInvalidRoot() {
    val config = Config(listOf(Profile("name", $$"${exception}s")))
    val result = EnvVarReplacer.replaceEnvVars(config)
    assertThat(result.exceptionOrNull()).hasMessage("fail")
  }
  ...
  fun replaceEnvVarsShouldReplaceEnvVarInIncludeProfiles() {
    val config = Config(listOf(Profile("name", "root", includeProfiles = listOf($$"${include1}s", $$"${include2}s"))))
    val result = EnvVarReplacer.replaceEnvVars(config)
    assertThat(result.exceptionOrNull()).isNull()
  ...
  fun replaceEnvVarsShouldReturnErrorIfInvalidIncludeProfiles() {
    val config = Config(listOf(Profile("name", "root", includeProfiles = listOf($$"${exception}s", $$"${exception}s", $$"${exception}s"))))
    val result = EnvVarReplacer.replaceEnvVars(config)
  ...
  fun replaceEnvVarsShouldReplaceEnvVarInInheritProfiles() {
    val config = Config(listOf(Profile("name", "root", inheritProfiles = listOf($$"${inherit1}s", $$"${inherit2}s"))))
    val result = EnvVarReplacer.replaceEnvVars(config)
    assertThat(result.exceptionOrNull()).isNull()
  ...
  fun replaceEnvVarsShouldReturnErrorIfInvalidInheritProfiles() {
    val config = Config(listOf(Profile("name", "root", inheritProfiles = listOf($$"${exception}s", $$"${exception}s", $$"${exception}s"))))
    val result = EnvVarReplacer.replaceEnvVars(config)
    val exception = result.exceptionOrNull()
    assertThat(exception).hasMessage("fail")
    assertThat(exception!!.suppressed).zipSatisfy(arrayOf("fail", "fail")) { a, b ->
  ...
  fun replaceEnvVarsShouldReplaceEnvVarInInclude() {
    val config = Config(listOf(Profile("name", "root", listOf($$"${include1}s", $$"${include2}s"))))
    val result = EnvVarReplacer.replaceEnvVars(config)
    assertThat(result.exceptionOrNull()).isNull()
  ...
  fun replaceEnvVarsShouldReturnErrorIfInvalidInclude() {
    val config = Config(listOf(Profile("name", "root", listOf($$"${exception}s", $$"${exception}s", $$"${exception}s"))))
    val result = EnvVarReplacer.replaceEnvVars(config)
    val exception = result.exceptionOrNull()
    assertThat(exception).hasMessage("fail")
    assertThat(exception!!.suppressed).zipSatisfy(arrayOf("fail", "fail")) { a, b ->
  ...
  fun replaceEnvVarsShouldReplaceEnvVarInIgnore() {
    val config = Config(listOf(Profile("name", "root", ignore = listOf($$"${ignore1}s", $$"${ignore2}s"))))
    val result = EnvVarReplacer.replaceEnvVars(config)
    assertThat(result.exceptionOrNull()).isNull()
  ...
  fun replaceEnvVarsShouldReturnErrorIfInvalidIgnore() {
    val config = Config(listOf(Profile("name", "root", ignore = listOf($$"${exception}s", $$"${exception}s", $$"${exception}s"))))
    val result = EnvVarReplacer.replaceEnvVars(config)
    val exception = result.exceptionOrNull()
    assertThat(exception).hasMessage("fail")
    assertThat(exception!!.suppressed).zipSatisfy(arrayOf("fail", "fail")) { a, b ->
