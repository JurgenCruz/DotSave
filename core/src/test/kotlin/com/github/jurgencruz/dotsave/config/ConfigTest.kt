...
class ConfigTest {
  ...
  fun validateShouldReturnErrorIfProfilesContainDuplicateNames() {
    val config = Config(
      listOf(
        Profile("test", "/root", listOf("a")),
        Profile("test", "/root2", listOf("a"))
      )
    )
    val result = runCatching { config.validate() }
  ...
  fun validateShouldReturnErrorIfProfilesIncludeMissingProfiles() {
    val config = Config(
      listOf(
        Profile("test", "/root", includeProfiles = listOf("child")),
        Profile("test2", "/root2", listOf("a"))
      )
    )
    val result = runCatching { config.validate() }
  ...
  fun validateShouldReturnErrorIfProfilesInheritMissingProfiles() {
    val config = Config(
      listOf(
        Profile("test", "/root", inheritProfiles = listOf("child")),
        Profile("test2", "/root2", listOf("a"))
      )
    )
    val result = runCatching { config.validate() }
  ...
  fun validateShouldReturnConfigIfValid() {
    val config = Config(
      listOf(
        ...
        Profile("include", "/root2", listOf("b")),
        Profile("inherit", "/root3", listOf("c"))
      )
    )
    val result = runCatching { config.validate() }
  ...
  fun selectProfileShouldReturnSelectedProfileByName() {
    val config = Config(
      listOf(
        Profile("program1", "/root1"),
        Profile("program2", "/root2", listOf("file3", "folder4"))
      )
    )
